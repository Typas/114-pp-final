import torch
from sa_v5 import sa_forward_v5

def patch_vit_pytorch_attention_sa_v5(model):
    patched = 0
    ...
    for name, m in model.named_modules():
        ...
        def forward_sa_v5(self, x):
            ...
            out = sa_forward_v5(q.contiguous(), k.contiguous(), v.contiguous(), scale)  # (b,h,n,d)
            out = out.transpose(1, 2).reshape(b, n, -1).contiguous()

            out = out.float()
            return self.to_out(out)

        m.forward = forward_sa_v5.__get__(m, m.__class__)
        patched += 1
        print(f"[INFO] Patched Attention with SA v5 (Q_ROWS=128): {name}")
