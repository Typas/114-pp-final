/**
 * Self-Attention v4 - Optimized WMMA with multiple warps
 *
 * Key optimizations over v3:
 * 1. 4 warps per block (vs 1 in v3) for better parallelism
 * 2. Each warp processes different Q rows independently
 * 3. Better memory coalescing for K/V loads
 */
...
constexpr int Q_ROWS = NUM_WARPS * Q_ROWS_PER_WARP;  // 64 query rows per block
...
sa_forward_v4_kernel(
    ...
    // Shared memory layout (shared across all warps):
    // Kc  : [64,64] half col_major for WMMA
    // V2  : [64,32] half2 row_major
    extern __shared__ unsigned char smem[];
    half*  Kc   = (half*)smem;                        // 64*64 = 8KB
    half2* V2   = (half2*)(Kc + D * TK);              // 64*32*4 = 8KB

    // Per-warp private storage in registers
    // Qs in registers: each warp loads its own Q tile
    half Qs_reg[Q_ROWS_PER_WARP * D / 32];  // Each lane holds part of Q
    ...
    // Load Q tile into shared memory (cooperatively by all threads)
    // Each warp needs its own Q[16x64] - we'll use warp-local shared memory
    __shared__ half Qs[NUM_WARPS][WM * D];  // 4 * 16 * 64 = 8KB
    ...
    for (int t0 = 0; t0 < N; t0 += TK) {
        ...
        // Per-warp shared memory for WMMA output
        __shared__ float Sbuf[NUM_WARPS][WM * WN];  // 4 * 16 * 16 = 4KB
...
torch::Tensor sa_forward_v4(torch::Tensor Q, torch::Tensor K, torch::Tensor V, double scale) {
    ...
    int N = (int)Q.size(2);
    int d = (int)Q.size(3);
    TORCH_CHECK(d == 64, "This v4 kernel assumes head dim D=64.");
    ...
    size_t shmem =
        ...
        (4ull * 16 * 64 * sizeof(half)) +   // Qs
        (4ull * 16 * 16 * sizeof(float));   // Sbuf

    sa_forward_v4_kernel<<<grid, block, shmem>>>(
