...
def patch_vit_pytorch_attention_sa_v4(model):
    patched = 0
    ...
    for name, m in model.named_modules():
        ...
        def forward_sa_v4(self, x):
            ...
            out = sa_forward_v4(q.contiguous(), k.contiguous(), v.contiguous(), scale)  # (b,h,n,d)
            out = out.transpose(1, 2).reshape(b, n, -1).contiguous()

            out = out.float()
            return self.to_out(out)

        m.forward = forward_sa_v4.__get__(m, m.__class__)
        patched += 1
        print(f"[INFO] Patched Attention with SA v4 (Optimized N=197): {name}")
